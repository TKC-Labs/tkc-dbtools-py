#!/usr/bin/env bash
#
# This script deploys the example databases to the local machine using Docker
#
set -euo pipefail
...
cleanup() {
    ...
    docker stop devdb proddb stagedb
    echo
    echo "Removing containers..."
    docker rm devdb proddb stagedb
    echo
    echo "Removing volumes..."
    docker volume rm example_devdb example_proddb example_stagedb
    echo
...
for cmd in docker mysql mysqladmin; do
    case $cmd in
        docker)
...
docker volume create example_devdb
docker volume create example_proddb
docker volume create example_stagedb
echo

# Dev environment
echo "Creating devdb container..."
docker run -d --name devdb --hostname devdb \
...
docker run -d --name proddb --hostname proddb \
    -e MYSQL_ROOT_PASSWORD=root \
...
docker run -d --name stagedb --hostname stagedb \
    -e MYSQL_ROOT_PASSWORD=root \
...
mysql --defaults-file=$(dirname "$0")/my-login.cnf -h 127.0.0.1 -P 3307 -e "SELECT User,Host FROM mysql.user WHERE Host NOT IN ('%', '127.0.0.1')"

echo
echo "Users deployed to proddb:"
mysql --defaults-file=$(dirname "$0")/my-login.cnf -h 127.0.0.1 -P 3306 -e "SELECT User,Host FROM mysql.user WHERE Host NOT IN ('%', '127.0.0.1')"

echo
echo "Users deployed to stagedb:"
mysql --defaults-file=$(dirname "$0")/my-login.cnf -h 127.0.0.1 -P 3308 -e "SELECT User,Host FROM mysql.user WHERE Host NOT IN ('%', '127.0.0.1')"
